...
const supabase = __require("@supabase/supabase-js");
...
function send_to_webhooks(jsonMessage) {
    secrets["discord-webhooks"].forEach((hook_url) => {
        ax.post(hook_url, jsonMessage);
    });
}
function make_request_payload(subject, user, notes, log_link) {
    let data = {
...
async function make_request_payload_entry(entry) {
    let logUrl;
...
async function upload_logs(logText) {
    ...
    let paste_url = resp.data;
    console.log("Pastebin URL:", paste_url);
    return paste_url;
}
function test() {
    send_to_webhooks(make_request_payload("Test", "reddust9#4805", "This is a test\nHope this works", "https://example.com/"));
}
